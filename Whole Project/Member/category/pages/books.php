   ...
   <div class="sidebar">
    ...
    <div class="main-content">
        ...
        <div class="categories d-flex flex-wrap justify-content-center gap-3 mt-4">
            <a href="books.php?genre=Science"><button type="button" class="btn btn-link" style="font-weight: bold; font-size:large; margin-top: -4px">Science</button></a>
    ...
    <div class="book-grid">
    <?php
        // $SELECTS_ALL_SCIENCE_BOOK = mysqli_query($conn, "SELECT * FROM books WHERE genre ='SCIENCE' AND status= 'Approved'");
        $SELECTS_ALL_SCIENCE_BOOK = mysqli_query($conn, "SELECT * FROM books WHERE genre ='$BOOKgenre'");
        if(mysqli_num_rows($SELECTS_ALL_SCIENCE_BOOK) > 0){
        while ($get_BOOKS = mysqli_fetch_assoc($SELECTS_ALL_SCIENCE_BOOK) ) {
            # code...
       
    ?>
        <button class="book"><a href="../pages/view_books.php?bookid=<?= $get_BOOKS['book_id']?>&genre=<?=$get_BOOKS['genre']?>">
             <img src="data:image/jpeg;base64, <?=base64_encode($get_BOOKS['front_cover'])?>" />
            <h4><?= $get_BOOKS['title']?></h4>
            <p>
                <?php
                    $authorname = $get_BOOKS['editor_id'];
                    $selects_the_name = mysqli_query($conn, "SELECT first_name FROM users WHERE user_id= $authorname");
        ...
        <?php
                ...
                echo "no available books for this genre";
            }
            ?>
