...
<body>
    ...
    <form method="GET" id="search-form" class="search-bar">
        <a href="#" type="button"><i id="mic-icon" class="fa-solid fa-microphone"></i></a>
        <input 
            type="search" 
            name="search" 
            id="searchbar-field" 
        ...
        />
        <input type="hidden" name="order" id="order-hidden" value="<?= htmlspecialchars($orderParam) ?>">
        <input type="hidden" name="sort" id="sort-hidden" value="<?= htmlspecialchars($sortParam) ?>">
        <a href="#" id="search-icon" type="button"><i class="fa-solid fa-magnifying-glass"></i></a>
    </form>

    <div class="navigations">
        <select name="Order" id="order-dropdown">
            <option value="Ascending" <?= $orderParam === 'Ascending' ? 'selected' : '' ?>>ASC</option>
            <option value="Descending" <?= $orderParam === 'Descending' ? 'selected' : '' ?>>DESC</option>
        </select>
        <select name="Sort Type" id="sort-type-dropdown">
            <option value="" <?= empty($sortParam) ? 'selected' : '' ?>>DEFAULT</option>
            <option value="DOWNLOAD" <?= $sortParam === 'DOWNLOAD' ? 'selected' : '' ?>>DOWNLOAD</option>
            <option value="A - Z" <?= $sortParam === 'A - Z' ? 'selected' : '' ?>>A - Z</option>
        </select>
        <ul>
            <li class="genre-selected"><a href="#">Science</a></li>
            <li><a href="Book_Category/Editor-Books-Novel.php">Novel</a></li>
            <li><a href="Book_Category/Editor-Books-Mystery.php">Mystery</a></li>
            <li><a href="Book_Category/Editor-Books-Narrative.php">Narrative</a></li>
            <li><a href="Book_Category/Editor-Books-Fiction.php">Fiction</a></li>
            <li><a href="Book_Category/Editor-Books-History.php">History</a></li>
        ...
        <tbody>
            ...
            if ($result_books->num_rows > 0) {
                ...
                while ($row = $result_books->fetch_assoc()) {
                    ...
                    echo "<td><a href='Editor-Books-View.php?book_id=" . intval($row['book_id']) . "'><button class='view-button'>View</button></a></td>";
                    echo "</tr>";
                }
            } else {
